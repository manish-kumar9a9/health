...
namespace Woomagestore\Payu\Model;
...
class Payu extends \Magento\Payment\Model\Method\AbstractMethod {
     ...
     */
      public function __construct(
        ...
        \Woomagestore\Payu\Helper\Payu $helper,
    ...
    public function postProcessing(\Magento\Sales\Model\Order $order,
        ...
        $writer = new \Zend\Log\Writer\Stream(BP . '/var/log/payu.log');
        $logger = new \Zend\Log\Logger();
        $logger->addWriter($writer);
        $logger->info('PAy U pay U pay U pay U pay U');
        ...
        $logger->info('Saving Payu Response mihpayid:'.$response['mihpayid'].', txnID:'.$response['mihpayid']);
        $order->setMaxPayuid($response['mihpayid']);
        $order->setMaxPayuTranc($response['txnid']);
